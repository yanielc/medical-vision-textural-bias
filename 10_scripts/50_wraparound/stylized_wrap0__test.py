...
from filters_and_operators import ConvertToMultiChannelBasedOnBratsClassesd, WrapArtifactd 
...
JOB_NAME = f"stylized_model_wrap{PP}"
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                print('val_labels slice', val_labels[:,0:1].size())
...
plt.savefig(root_dir + f'meanValScore_per_label_{JOB_NAME}.png')
plt.show()
...
np.savetxt(os.path.join(working_dir, f'metric_values_tc_{JOB_NAME}.txt'), np.array(metric_values_tc))
np.savetxt(os.path.join(working_dir, f'metric_values_wt_{JOB_NAME}.txt'), np.array(metric_values_wt))
np.savetxt(os.path.join(working_dir, f'metric_values_et_{JOB_NAME}.txt'), np.array(metric_values_et))
