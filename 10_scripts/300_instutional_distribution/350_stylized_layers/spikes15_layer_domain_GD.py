...
from filters_and_operators import WholeTumorTCGA, RandKSpaceSpikeNoise 
...
JOB_NAME = f"spikes{INTENSITY}_layer_GD_model_sourceDist_4mods_WT"
...
model = Spikes_UNet(INTENSITY).to(device)
...
@torch.no_grad()
def Gibbs_GD(inputs, labels, model, h = 0.05, learning_rate = 0.1):
    """Function to update Gibbs layer via finite different SG"""
#     with torch.no_grad():
    old_intensity = model.spike.intensity.clone()
    # loss at alpha
    outputs_0 = model(inputs)
    loss_0 = loss_function(outputs_0, labels)
    # loss at perturbed alpha
    model.spike.intensity = old_intensity + h
    outputs_h = model(inputs)
    loss_h = loss_function(outputs_h, labels)
    # approximate gradient
    delta = (loss_h - loss_0) / h
    # update alpha and model
    model.spike.intensity = old_intensity - learning_rate * delta
    return loss_0.detach().item(), model.spike.intensity.item()
...
for epoch in range(max_epochs):
    ...
    for batch_data in train_loader:
        ...
        gibbs_values.append(model.spike.intensity.detach().item())
...
np.savetxt(os.path.join(working_dir, f'spikes_trajectory_{JOB_NAME}.txt'), np.array(gibbs_values))
############################################################################
